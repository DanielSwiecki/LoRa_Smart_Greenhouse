...
#            print("RXcb receive: ", utime.ticks_ms())
    ...
    def __init__(self, configuration):
        ...
        # self.SXRadio = SX1262(spi_bus=0, clk=2, mosi=3, miso=4, cs=5, irq=6, rst=7, gpio=8)

        self.SXRadio = SX1262(spi_bus=SPI_BUS, clk=CLK, mosi=MOSI, miso=MISO, cs=CS,irq=DIO1,rst=RESET,gpio=BUSY)
        ...
        self.SXRadio.begin(freq=self.freqList[self.currentFreq], bw=self.DRList[self.currentDR][0], sf=self.DRList[self.currentDR][1], cr=8, syncWord=SX126X_SYNC_WORD_PUBLIC,
         power=self.TXPowerTable[self.currentPower], currentLimit=60.0, preambleLength=8,
...
#        self.currentFreq = 0
    ...
    def otaa(self):
        ...
        RX1Timeout = (self.JOIN_ACCEPT_DELAY1 + 10) * 1000
        RX2Timeout = (self.JOIN_ACCEPT_DELAY2 + 10) * 1000
    ...
    #        print("Time elapsed since start: ", utime.ticks_diff(utime.ticks_ms, startTime))
            ...
            # Timeout
            if (utime.ticks_diff(utime.ticks_ms(), startTime) > (RX1Timeout)):
                print("RX1 Timeout!") 
                break
        ...
        if (self.Connected == False):
            ...
            while True:
                ...
                # Timeout
                if (utime.ticks_diff(utime.ticks_ms(), startTime) > (RX2Timeout)):
                    print("RX2 Timeout!")
                    break
    #    except:
